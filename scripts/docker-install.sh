...
### Variables
###
PHP_FPM_POOL_CONF="/etc/opt/remi/php72/php-fpm.d/www.conf"
PHP_FPM_CONF="/etc/opt/remi/php72/php-fpm.conf"
...
grep -q '^cgi.fix_pathinfo = 0$' "${PHP_INI}"
if [ "$( grep -c '^cgi.fix_pathinfo[[:space:]]*=' ${PHP_INI} )" != "1" ]; then
	exit 1
fi

# Needed for PHP to read out docker-compose variables
run "sed -i'' 's/^variables_order[[:space:]]*=.*$/variables_order = EGPCS/g' ${PHP_INI}"
grep -q '^variables_order = EGPCS$' "${PHP_INI}"
if [ "$( grep -c '^variables_order[[:space:]]*=' ${PHP_INI} )" != "1" ]; then
	exit 1
fi
